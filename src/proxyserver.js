...
function getproxylist() {
  return new Promise(async (resolve, reject) => {
    ...
    var layoutVisible = await page
      .waitForSelector(".table-container table.sortable tbody")
      .catch(ex => {
    ...
    var proxyModelArray = await page.evaluate(async () => {
      ...
      let list = document.querySelectorAll(
        ".table-container table.sortable tbody tr"
      );
      if (!list) {
        return;
      }
...
async function getProxyBrowser(checkurl, rtc = 3) {
  return new Promise(async (resolve, reject) => {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      var proxyOption =
        proxyserver.version.toLowerCase() +
        "://" +
        proxyserver.ip +
        ":" +
        proxyserver.port;
